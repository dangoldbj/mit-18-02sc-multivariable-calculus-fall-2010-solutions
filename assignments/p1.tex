...
\begin{solution}
    \enum {
        \item $w1 = w \cos(\alpha)$. Since $0 \leq \alpha \leq \pi/2$, this implies $w1$ won't have any $-i$ component.

        \item $p = w1 \cos(\beta) =  w \cos(\alpha) \cos(\beta)$.
            Now projection of $p$ in x-axis is $p \cos(\alpha + \beta)$.
            Since $ 0 \leq \alpha, \beta \leq \pi/2$, $\cos(\alpha + \beta) < 0$  when,
            $\alpha + \beta > \pi/2$
    }
\end{solution}
